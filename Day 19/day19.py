...
def findGeodes(blueprint, minutes):
...
def part1(blueprints):
    ...
    for i, blueprint in enumerate(blueprints):
        geodes = findGeodes(blueprint, minutes=24)
...
def part2(blueprints):
    totalVal = 1
    for i in range(3):
        x = findGeodes(blueprints[i], minutes=32)
        totalVal *= x
        print(x)
...
def main():
    ...
    print('Part 2 -', part2(blueprints)) # 37191 (Just over 3 mins) - Off by one error [33, 23, 48](36432) should be [33, 23, 49](37191) - No idea where the bug is
